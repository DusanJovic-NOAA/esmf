...
macro (copy_storeh)
   execute_process(
        COMMAND mkdir -p ${PROJECT_BINARY_DIR}/src/include/
        COMMAND cp ${STOREH} ${PROJECT_BINARY_DIR}/src/include/
...
macro (run_cppF90_to_F90)
    foreach(out_file ${AUTOGEN})
        ...
        execute_process(
            COMMAND ${PROJECT_SOURCE_DIR}/cmake/scripts/cppF90_to_F90.sh ${CMAKE_CURRENT_SOURCE_DIR}/${in_file} ${out_file} ${PROJECT_BINARY_DIR}/src/include
...
${arg_PRE_RUN_SCRIPT}
")
         file(WRITE "${PROJECT_BINARY_DIR}/tests_rundir/${PRE_RUN_SCRIPT_FILE}" ${RUN_SCRIPT})
         file(CHMOD "${PROJECT_BINARY_DIR}/tests_rundir/${PRE_RUN_SCRIPT_FILE}" PERMISSIONS OWNER_EXECUTE OWNER_WRITE OWNER_READ)
    endif()
...
${arg_POST_RUN_SCRIPT}
")
         file(WRITE "${PROJECT_BINARY_DIR}/tests_rundir/${POST_RUN_SCRIPT_FILE}" ${RUN_SCRIPT})
         file(CHMOD "${PROJECT_BINARY_DIR}/tests_rundir/${POST_RUN_SCRIPT_FILE}" PERMISSIONS OWNER_EXECUTE OWNER_WRITE OWNER_READ)
    endif()
    ...
    set(CMAKE_RUNTIME_OUTPUT_DIRECTORY "${PROJECT_BINARY_DIR}/tests_rundir")

    target_sources(${arg_NAME} PRIVATE ${arg_SOURCES})
    target_link_libraries(${arg_NAME} PRIVATE esmf)

    add_test(NAME ${arg_NAME}
             COMMAND ${PROJECT_SOURCE_DIR}/cmake/scripts/run_test.sh
                                         ${arg_NAME}
                                         ${arg_NP}
                                         ${PRE_RUN_SCRIPT_FILE}
                                         ${POST_RUN_SCRIPT_FILE}
             WORKING_DIRECTORY ${PROJECT_BINARY_DIR}/tests_rundir)
